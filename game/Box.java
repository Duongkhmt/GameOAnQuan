...
public class Box implements ActionListener {
	...
	void create() { // pos = 0~11
		...
		boxBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				// Team 0 (người chơi): chỉ được chọn ô 0-4 hoặc ô quan 5
				if (team == 0 && (pos >= 0 && pos <= 5) && num > 0) {
					showArrow();
				}
				// Team 1 (AI): chỉ được chọn ô 6-10 hoặc ô quan 11
				if (team == 1 && (pos >= 6 && pos <= 11) && num > 0) {
					showArrow();
				}
			}
		});
	...
	void resetStone() {
		...
		if (pos < 5)
			for (int i = 0; i < 5; i++)
				isStone[5 * pos + i] = true;
		else if (pos == 5)
			for (int i = 0; i < 10; i++)
				isStone[25 + i] = true;
		else if (pos < 11)
			for (int i = 0; i < 5; i++)
				isStone[5 * (pos + 1) + i] = true;
		else
			for (int i = 0; i < 10; i++)
				isStone[60 + i] = true;
